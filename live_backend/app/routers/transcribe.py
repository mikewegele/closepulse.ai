...
async def transcribe(file: UploadFile = File(...), x_conversation_id: str | None = Header(default=None)):
...
async def transcribe_by_ext(ext_id: str) -> str:
    wavs = sorted(recordings_dir.glob(f"{ext_id}-*-spk.wav"))
    if not wavs:
        return ""
    latest = wavs[-1]
